...
export async function handleGeneratePdf(job) {
    ...
    if (row.matchStatus === 'PENDING' || row.matchStatus === 'PROCESSING') {
        console.warn(`[pdf] Cannot generate PDF — review ${investigationId} not yet scored (status: ${row.matchStatus})`);
        return;
    }
    try {
        ...
        console.log(`[pdf] Generated dispute packet ${caseId} for review ${investigationId}`);
    }
    catch (err) {
