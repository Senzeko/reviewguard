...
export async function dashboardRoutes(fastify) {
    ...
    fastify.get('/stats', async (request, reply) => {
        ...
        for (const row of rows) {
            ...
            if (row.matchStatus === 'PENDING' || row.matchStatus === 'PROCESSING')
                pending += c;
            else if (row.matchStatus === 'VERIFIED')
                verified += c;
            else if (row.matchStatus === 'MISMATCH')
                mismatch += c;
            else if (row.matchStatus === 'NO_RECORD')
                noRecord += c;
        }
    ...
    fastify.get('/investigations', async (request, reply) => {
        if (!request.user?.merchantId) {
            return reply.status(403).send({ error: 'No merchant linked to account' });
        }
        const merchantId = request.user.merchantId;
        ...
        if (statusFilter && ['VERIFIED', 'MISMATCH', 'NO_RECORD', 'PENDING', 'PROCESSING'].includes(statusFilter)) {
            conditions.push(eq(reviewsInvestigation.matchStatus, statusFilter));
        }
