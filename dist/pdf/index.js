...
import { embedFonts, newPage, MARGIN, CONTENT_WIDTH, COLORS, FONT_SIZES, drawText, drawLine, } from './layout.js';
...
// ── Case ID generation ───────────────────────────────────────────────────────
export function generateCaseId(reviewPublishedAt, posTransactionId) {
...
// ── Data assembly ────────────────────────────────────────────────────────────
export async function assembleEvidencePacket(investigationId) {
    ...
    if (row.matchStatus === 'PENDING' || row.matchStatus === 'PROCESSING') {
        throw new Error(`Cannot generate PDF — review ${investigationId} not yet scored (status: ${row.matchStatus})`);
    }
    ...
    const m = merchant[0];
    if (!m)
        throw new Error(`Merchant ${row.merchantId} not found`);
    let matchedTxn = null;
    ...
    const rawFb = row.factorBreakdown;
    let fb = null;
    if (rawFb) {
    ...
    const caseId = row.caseId ?? generateCaseId(row.reviewPublishedAt, matchedTxn?.posTransactionId ?? null);
    return {
        caseId,
...
// ── PDF generation ───────────────────────────────────────────────────────────
export async function generateDisputePacket(investigationId) {
    ...
    for (let i = 0; i < totalPages; i++) {
        ...
        drawText(page, `ReviewGuard AI \u00B7 Case ${packet.caseId}`, MARGIN, footerY, { font: fonts.regular, size: FONT_SIZES.tiny, color: COLORS.midGray });
        ...
        drawText(page, centerText, MARGIN + (CONTENT_WIDTH - centerWidth) / 2, footerY, { font: fonts.regular, size: FONT_SIZES.tiny, color: COLORS.midGray });
        const pageText = `Page ${i + 1} of ${totalPages}`;
        const pageWidth = fonts.regular.widthOfTextAtSize(pageText, FONT_SIZES.tiny);
        drawText(page, pageText, MARGIN + CONTENT_WIDTH - pageWidth, footerY, { font: fonts.regular, size: FONT_SIZES.tiny, color: COLORS.midGray });
    }
