...
export async function storePdf(investigationId, merchantId, caseId, pdfBytes) {
...
export async function retrievePdf(investigationId) {
    const rows = await db
        .select({
        ...
        .limit(1);
    const row = rows[0];
    if (!row?.pdfPath || !row.caseId || !row.pdfGeneratedAt)
        return null;
...
export async function pdfExists(investigationId) {
    const rows = await db
