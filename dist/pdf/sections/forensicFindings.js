 ...
 */
import { MARGIN, CONTENT_WIDTH, COLORS, FONT_SIZES, drawText, drawRect, drawBadge, drawLine, drawSectionHeader, checkPageBreak, } from '../layout.js';
...
export function renderForensicFindings(ctx, packet) {
    ctx = drawSectionHeader(ctx, '2. FORENSIC AUDIT FINDINGS');
    const providerName = packet.matchedTransaction?.posProvider ?? 'POS';
    ctx.y = drawText(ctx.page, `Data retrieved via direct API integration with POS (${providerName}).`, MARGIN, ctx.y, { font: ctx.fonts.regular, size: FONT_SIZES.body, color: COLORS.darkGray });
    ctx.y -= 8;
    const factors = [
        {
            name: 'Identity Match',
    ...
    for (const factor of factors) {
        ...
        // Level badge
        drawBadge(ctx.page, factor.result.level, MARGIN + CONTENT_WIDTH - 56, ctx.y - 2, dotColor, ctx.fonts.bold);
        ctx.y -= 18;
        // Points
        ...
        if (factor.isLineItem && packet.llmInferenceFlag) {
            ...
            ctx.y -= 4;
            ctx.y = drawText(ctx.page, `LLM extracted: ${li.llmExtractedItems.join(', ') || 'None'}`, MARGIN + 16, ctx.y, { font: ctx.fonts.regular, size: 9, color: COLORS.darkGray });
            ctx.y = drawText(ctx.page, `POS items: ${li.posItems.join(', ') || 'None'}`, MARGIN + 16, ctx.y, { font: ctx.fonts.regular, size: 9, color: COLORS.darkGray });
            ctx.y = drawText(ctx.page, `Matched: ${li.matchedItems.join(', ') || 'None'}`, MARGIN + 16, ctx.y, { font: ctx.fonts.regular, size: 9, color: COLORS.darkGray });
        }
        ctx.y -= 4;
    ...
    const barColor = packet.confidenceScore >= 75
        ? COLORS.green
    ...
    ctx.y = drawText(ctx.page, `Composite score: ${packet.confidenceScore} / 100  ${ptsBreakdown}`, MARGIN, ctx.y, { font: ctx.fonts.regular, size: 9, color: COLORS.midGray });
    ctx.y -= 16;
    return ctx;
}
//# sourceMappingURL=forensicFindings.js.map