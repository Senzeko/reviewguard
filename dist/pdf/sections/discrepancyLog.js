 ...
 */
import { MARGIN, CONTENT_WIDTH, COLORS, FONT_SIZES, drawText, drawRect, drawBadge, drawSectionHeader, checkPageBreak, wrapText, formatDateTime, } from '../layout.js';
export function renderDiscrepancyLog(ctx, packet) {
    ...
    if (!packet.matchedTransaction) {
        ...
        const msg = `No matching transaction found within the 14-day window surrounding ` +
        ...
        const discrepancies = li.llmExtractedItems.filter((item) => !li.matchedItems.some((m) => m.toLowerCase() === item.toLowerCase()));
        if (discrepancies.length === 0) {
            // All items matched
            ctx = checkPageBreak(ctx, 30);
            ctx.y = drawText(ctx.page, 'No item discrepancies detected. All mentioned items appear in POS records.', MARGIN, ctx.y, { font: ctx.fonts.regular, size: FONT_SIZES.body, color: COLORS.green });
            ctx.y -= 12;
        }
        else {
            ...
            for (const item of discrepancies) {
                ...
                const bodyMsg = `Review mentions "${item}". Transaction ${packet.matchedTransaction.posTransactionId} ` +
...
//# sourceMappingURL=discrepancyLog.js.map