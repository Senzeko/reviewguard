...
if (env.NODE_ENV !== 'development') {
  throw new Error(
    '[seed] Seed script only runs in development. NODE_ENV is currently: ' +
      env.NODE_ENV,
  );
}
...
async function seed(): Promise<void> {
  ...
  if (!merchant) throw new Error('[seed] Merchant insert returned no rows');
  ...
  for (const tx of txRows) {
    ...
    if (!inserted) throw new Error(`[seed] Transaction insert failed for ${tx.posTransactionId}`);
    insertedTxIds.push(inserted.id);
    console.log(
      `[seed] ✓ Transaction inserted: ${inserted.id}  (${tx.posTransactionId}, customer: "${tx.customerName}", closed: ${tx.closedAt.toISOString()})`,
    );
  }
  ...
  const [review] = await db
    ...
    .values({
      ...
      reviewText:
        'The Fish Tacos here are absolutely incredible — fresh, perfectly seasoned, ' +
  ...
  if (!review) throw new Error('[seed] Review investigation insert returned no rows');
  console.log(
    `[seed] ✓ Review investigation inserted: ${review.id}  (google_review_id: "${review.googleReviewId}", reviewer: "${review.reviewerDisplayName}")`,
  );
