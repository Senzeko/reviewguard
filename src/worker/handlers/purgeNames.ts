...
export async function handlePurgeNames(): Promise<void> {
  const result = await db.execute<{ purge_expired_name_plain_temp: number }>(
    sql`SELECT purge_expired_name_plain_temp() as purge_expired_name_plain_temp`,
  );
