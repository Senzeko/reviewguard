...
export function getCloverOAuthUrl(
  merchantId: string,
  baseUrl: string,
): string {
...
export async function syncCloverTransactions(
  merchantId: string,
  windowDays: number,
): Promise<number> {
  ...
  let url:
    | string
    | null = `${CLOVER_BASE}/v3/merchants/${merchant.cloverMerchantId}/orders?filter=clientCreatedTime>${epochMs}&expand=lineItems,customers&limit=100`;

  interface CloverListResponse {
    elements?: CloverOrder[];
    links?: Array<{ rel: string; href: string }>;
  }

  async function fetchPage(pageUrl: string): Promise<CloverListResponse> {
    const resp = await axios.get<CloverListResponse>(pageUrl, {
  ...
  try {
    while (url) {
      ...
      const orders = (data.elements ?? []).filter(
        (o: CloverOrder) => o.state === 'paid',
      );
    ...
    if (axios.isAxiosError(err)) {
      if (err.response?.status === 401) {
        console.warn(
          `[clover] Token expired/revoked for merchant ${merchantId} — deactivating`,
        );
        await db
          .update(merchants)
      ...
      console.error(
        `[clover] API error ${err.response?.status}:`,
        err.response?.data,
      );
    } else {
