...
export function getSquareOAuthUrl(merchantId: string, baseUrl: string): string {
  ...
  try {
    do {
      ...
      const resp = await axios.post<{
        orders?: SquareOrder[];
        cursor?: string;
      }>(`${SQUARE_BASE}/v2/orders/search`, body, {
    ...
    if (axios.isAxiosError(err)) {
      if (err.response?.status === 401) {
        console.warn(
          `[square] Token expired/revoked for merchant ${merchantId} — deactivating`,
        );
        await db
          .update(merchants)
      ...
      console.error(
        `[square] API error ${err.response?.status}:`,
        err.response?.data,
      );
    } else {
