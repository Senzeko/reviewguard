...
export function renderTimestampAudit(
  ...
  if (!packet.matchedTransaction) {
    ...
    drawText(
      ctx.page,
      `POS transaction closed \u2014 ${txn.posTransactionId}`,
      textX,
      event1Y - 14,
    ...
    drawText(
      ctx.page,
      `${txn.posProvider} \u00B7 ${formatCents(txn.transactionAmountCents)}`,
      textX,
      event1Y - 28,
      { font: ctx.fonts.regular, size: FONT_SIZES.small, color: COLORS.midGray },
    );
    ...
    drawText(
      ctx.page,
      `${packet.reviewerDisplayName} \u00B7 ${packet.reviewRating} stars`,
      textX,
      event2Y - 28,
      { font: ctx.fonts.regular, size: FONT_SIZES.small, color: COLORS.midGray },
    );
    ...
    ctx.y = drawText(
      ctx.page,
      `Window searched: ${formatDateTime(windowStart)} to ${formatDateTime(windowEnd)}`,
      MARGIN,
      ctx.y,
      { font: ctx.fonts.regular, size: FONT_SIZES.small, color: COLORS.midGray },
    );
    ctx.y = drawText(
      ctx.page,
      'Transactions found in window: 1',
      MARGIN,
      ctx.y,
      { font: ctx.fonts.regular, size: FONT_SIZES.small, color: COLORS.midGray },
    );
