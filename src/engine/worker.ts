...
import { onScoringComplete } from '../email/notify.js';
...
async function recoverStuckRows(): Promise<void> {
  ...
  const result = await db
    ...
    .where(
      and(
        eq(reviewsInvestigation.matchStatus, 'PROCESSING'),
        lt(reviewsInvestigation.updatedAt, fiveMinAgo),
      ),
    )
  ...
  if (result.length > 0) {
    console.log(
      `[engine] Recovered ${result.length} stuck PROCESSING reviews back to PENDING`,
    );
  }
}

async function pollOnce(): Promise<void> {
  if (_processing) return; // prevent overlapping polls
  _processing = true;
  ...
  try {
    ...
    for (const review of pending) {
      ...
      const windowStart = new Date(
        review.reviewPublishedAt.getTime() - 14 * 24 * 60 * 60 * 1000,
      );
      const windowEnd = new Date(
        review.reviewPublishedAt.getTime() + 1 * 24 * 60 * 60 * 1000,
      );
      ...
      const transactions = await db
        ...
        .where(
          and(
            eq(transactionsVault.merchantId, review.merchantId),
      ...
      // e. Log
      console.log(
        `[engine] Review ${review.id} \u2192 score ${result.confidence_score}, status ${result.match_status}`,
      );

      // f. SSE broadcast
      broadcastSSE(review.merchantId, 'review:scored', {
        investigationId: review.id,
        score: result.confidence_score,
        matchStatus: result.match_status,
      });

      // g. Email notification (fire-and-forget)
...
export async function startEngineWorker(): Promise<void> {
  if (_running) return;
  _running = true;
...
export function stopEngineWorker(): void {
  _running = false;
  if (_timer) {
