...
export class ForensicMatchEngine {
  ...
  async match(
    ...
    try {
      ...
      const now = new Date();
      const nameExpired =
        best.namePlainExpiresAt !== null && best.namePlainExpiresAt < now;
      const customerName =
        nameExpired || best.namePlainTemp === null
          ? null
          : best.namePlainTemp;

      // 4. Run all three factor computations sequentially
      const identityResult = computeIdentityScore(
        review.reviewerDisplayName,
        customerName,
      );

      const temporalResult = computeTemporalScore(
        review.reviewPublishedAt,
        best.closedAt,
      );

      const lineItemResult = await computeLineItemScore(
        review.reviewText,
        best.lineItems,
      );
  ...
  private selectBestTransaction(
    ...
    for (const txn of transactions) {
      const delta = Math.abs(
        review.reviewPublishedAt.getTime() - txn.closedAt.getTime(),
      );
      if (delta < bestDelta) {
        bestDelta = delta;
        best = txn;
  ...
  private computeCompositeScore(
    ...
    return Math.round(
      (identityScore * 0.4 + temporalScore * 0.3 + lineItemScore * 0.3) * 100,
    );
  }
  ...
  private determineMatchStatus(
    score: number,
    identity: FactorResult,
    temporal: FactorResult,
    lineItem: FactorResult & { llm_extracted_items?: string[] },
    hasTransactions: boolean,
  ): ForensicMatchResult['match_status'] {
    // 1. No transactions
    if (!hasTransactions) return 'NO_RECORD';

    // 2. Line-item contradiction: items mentioned but not on receipt
    const extractedItems = (lineItem as LineItemFactorResult)
      .llm_extracted_items;
    if (
      lineItem.level === 'NONE' &&
      extractedItems &&
    ...
    const highCount = [identity, temporal, lineItem].filter(
      (f) => f.level === 'HIGH',
    ).length;
    if (score >= 75 && highCount >= 2) return 'VERIFIED';
  ...
  private safeErrorFallback(review: ReviewInput): ForensicMatchResult {
    return {
      confidence_score: 0,
