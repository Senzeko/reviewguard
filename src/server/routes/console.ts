...
export async function consoleRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get<{ Params: { investigationId: string } }>(
      ...
      if (review.matchedTransactionId) {
        ...
        if (txn) {
          matchedTransaction = {
            ...
            lineItems: txn.lineItems as LineItem[],
  ...
  fastify.post<{
    ...
    Body: { merchantUserId?: string; acknowledgedSections: number[] };
  }>(
    '/investigations/:investigationId/confirm',
    async (request, reply) => {
      const { investigationId } = request.params;
  ...
  fastify.get<{ Params: { investigationId: string } }>(
    '/investigations/:investigationId/pdf-status',
    async (request, reply) => {
      const { investigationId } = request.params;
