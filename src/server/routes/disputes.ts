...
export async function disputeRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post<{ Params: { investigationId: string } }>(
    '/disputes/:investigationId/export',
    async (request, reply) => {
  ...
  fastify.get<{ Params: { investigationId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply
        ...
        .header(
          'Content-Disposition',
          `attachment; filename="ReviewGuard-${result.caseId}.pdf"`,
        )
