...
export function validateEncryptionKey(): void {
  ...
  if (key.length !== 64) {
    throw new Error(
      `[ReviewGuard/secrets] ENCRYPTION_KEY must be exactly 64 hex characters (32 bytes). ` +
  ...
  if (!/^[0-9a-fA-F]+$/.test(key)) {
    throw new Error(
      `[ReviewGuard/secrets] ENCRYPTION_KEY must be a hex string (0-9, a-f). ` +
 ...
 */
export function hashName(name: string, salt: string): string {
...
if (
  ...
  console.log(
    `[secrets self-test] decrypt(encrypt("${plaintext}")) === "${recovered}"`,
  );
  ...
  if (!tamperDetected) {
    console.error(
      '[secrets self-test] FAILED: tampered ciphertext was not rejected!',
    );
    process.exit(1);
  }
